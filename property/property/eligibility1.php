...
<div class="tab-content col-md-10">
		...
		function eligiable()
        {
                var P1 = document.formval2.pr_amt2.value; 
                var rate1 = document.formval2.int_rate2.value;
                var n1 = document.formval2.period2.value;
                var r1 = rate1/(12*100); 
                var prate1 = (P1 * r1 * Math.pow((1+r1),n1*12))/(Math.pow((1+r1),n1*12)-1); 
                 var emi1 = Math.ceil(prate1 * 100) / 100; // 
				var existing = document.formval2.ExLoan.value;
				var existingLoan=(existing-(existing*50/100));
				...
				var prate2 = (incomereq * r1 * Math.pow((1+r1),n1*12))/(Math.pow((1+r1),n1*12)-1);                 
				 var emi2 = Math.ceil((prate2) * 100) / 100;
				...
				<form name="formval2" class="form-horizontal col-md-8" style="font-size:12px;">
					...
					<div class="form-group">
						...
						<div class="col-sm-8">
						<div class="input-group">
						<input type="number" class="form-control input-sm" id="input"  value="9.5" name="int_rate2">
						<span class="input-group-addon">%</span>
						</div>
						</div>
					</div>
